...
_DEFAULT_CMDS: dict[str, list[list[str]]] = {'py': [['ty', 'server'], ['pyright-langserver', '--stdio'], ['pylsp']], 'ts': [['typescript-language-server', '--stdio'], ['vtsls', '--stdio']], 'tsx': [['typescript-language-server', '--stdio'], ['vtsls', '--stdio']], 'js': [['typescript-language-server', '--stdio'], ['vtsls', '--stdio']], 'jsx': [['typescript-language-server', '--stdio'], ['vtsls', '--stdio']], 'html': [['vscode-html-language-server', '--stdio']], 'css': [['vscode-css-language-server', '--stdio']], 'json': [['vscode-json-language-server', '--stdio']]}
_EXT_TO_LANG: dict[str, str] = {'.py': 'py', '.pyw': 'py', '.ts': 'ts', '.tsx': 'tsx', '.js': 'js', '.mjs': 'js', '.cjs': 'js', '.jsx': 'jsx', '.html': 'html', '.htm': 'html', '.css': 'css', '.scss': 'css', '.less': 'css', '.json': 'json', '.jsonc': 'json'}
_LANG_ID: dict[str, str] = {'py': 'python', 'ts': 'typescript', 'tsx': 'typescriptreact', 'js': 'javascript', 'jsx': 'javascriptreact', 'html': 'html', 'css': 'css', 'json': 'json'}
_SYMBOL_KIND_NAME: dict[int, str] = {1: 'File', 2: 'Module', 3: 'Namespace', 4: 'Package', 5: 'Class', 6: 'Method', 7: 'Property', 8: 'Field', 9: 'Constructor', 10: 'Enum', 11: 'Interface', 12: 'Function', 13: 'Variable', 14: 'Constant', 15: 'String', 16: 'Number', 17: 'Boolean', 18: 'Array', 19: 'Object', 20: 'Key', 21: 'Null', 22: 'EnumMember', 23: 'Struct', 24: 'Event', 25: 'Operator', 26: 'TypeParameter'}
...
class LspClient:
    ...
    async def start(self, cwd: str) -> None:
        ...
        await self._request('initialize', {'processId': os.getpid(), 'rootUri': self._root_uri, 'workspaceFolders': [{'uri': self._root_uri, 'name': pathlib.Path(cwd).name}], 'capabilities': {'textDocument': {'synchronization': {'didOpen': True, 'didChange': True}, 'publishDiagnostics': {}, 'documentSymbol': {'hierarchicalDocumentSymbolSupport': True}, 'definition': {}, 'references': {}, 'hover': {}, 'rename': {}}, 'workspace': {'symbol': {}, 'workspaceFolders': True}}, 'initializationOptions': {}})
    ...
    async def _sync(self, path: str) -> str:
        ...
        if uri in self._open_files:
            ...
            await self._notify('textDocument/didChange', {'textDocument': {'uri': uri, 'version': ver}, 'contentChanges': [{'text': text}]})
        else:
            self._open_files[uri] = 1
            await self._notify('textDocument/didOpen', {'textDocument': {'uri': uri, 'languageId': lang_id, 'version': 1, 'text': text}})
        return uri

    async def document_symbols(self, path: str) -> list[dict]:
        uri = await self._sync(path)
        return _as_list(await self._request('textDocument/documentSymbol', {'textDocument': {'uri': uri}}))

    async def hover(self, path: str, line: int, char: int) -> str:
        uri = await self._sync(path)
        result = await self._request('textDocument/hover', {'textDocument': {'uri': uri}, 'position': {'line': line, 'character': char}})
        if not result:
            return ''
    ...
    async def definition(self, path: str, line: int, char: int) -> list[dict]:
        uri = await self._sync(path)
        return _as_list(await self._request('textDocument/definition', {'textDocument': {'uri': uri}, 'position': {'line': line, 'character': char}}))

    async def references(self, path: str, line: int, char: int, include_declaration: bool=True) -> list[dict]:
        uri = await self._sync(path)
        return _as_list(await self._request('textDocument/references', {'textDocument': {'uri': uri}, 'position': {'line': line, 'character': char}, 'context': {'includeDeclaration': include_declaration}}))

    async def rename(self, path: str, line: int, char: int, new_name: str) -> dict | None:
        uri = await self._sync(path)
        return await self._request('textDocument/rename', {'textDocument': {'uri': uri}, 'position': {'line': line, 'character': char}, 'newName': new_name})
        ...
        while True:
            ...
            if 'id' in msg:
                ...
                if fut and (not fut.done()):
                    if 'error' in msg:
                        fut.set_exception(LspError(msg['error'].get('message', str(msg['error']))))
                    else:
...
async def _get_or_start(ctx: dict, lang: str) -> LspClient | None:
    ...
    proc = await asyncio.create_subprocess_exec(*cmd, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL, cwd=cwd)
...
def _rebuild_hierarchy(flat: list[dict]) -> list[dict]:
    items = sorted(flat, key=lambda s: (_sym_range(s)[0], -(_sym_range(s)[1] - _sym_range(s)[0])))
...
def _get_hierarchy(client: LspClient, path: str) -> Any:
    ...
    async def _inner() -> list[dict]:
        ...
        return raw
    return _inner()
_BUILTIN_HOVER_PREFIXES = ('dict() ->', 'dict(mapping)', 'dict(iterable)', 'dict(**kwargs)', 'list() ->', 'list(iterable)', 'set() ->', 'set(iterable)', 'frozenset(', 'Built-in mutable sequence', 'Built-in immutable sequence', 'If no argument is given, the constructor', 'str(object', 'int(', 'float(', 'bool(', 'tuple(')
...
def _fmt_map(syms: list[dict], depth: int=0, max_depth: int=99, src_lines: list[str] | None=None, _skip: bool=False) -> list[str]:
    lines = []
    for sym in syms:
...
def _fmt_diagnostics(diags: list[dict], rel: str) -> str:
    ...
    for sev in sorted(by_sev):
        ...
        for d in sorted(by_sev[sev], key=lambda x: x.get('range', {}).get('start', {}).get('line', 0)):
            ...
            for ri in d.get('relatedInformation') or []:
                ...
                ri_line = ri.get('location', {}).get('range', {}).get('start', {}).get('line', 0) + 1
                parts.append(f'    note: {ri.get('message', '')}  ({ri_path}:{ri_line})')
    return '\n'.join(parts)
...
def _fmt_rename_edit(edit: dict, cwd: str) -> tuple[str, int]:
    ...
    def add(uri: str, edits: list[dict]) -> None:
        ...
        for e in edits:
            ...
            lines.append(f'  {fp}:{s.get('line', 0) + 1}:{s.get('character', 0) + 1}–{en.get('line', 0) + 1}:{en.get('character', 0) + 1}  →  {e.get('newText', '')!r}')
...
class LspParams(BaseModel):
    action: Literal['map', 'inspect', 'callers', 'definitions', 'problems', 'rename'] = Field(description="map: symbol hierarchy of a file or directory — classes, functions, methods with line spans. Use this first to orient yourself, then inspect the symbols that matter. inspect: full source + type signature + docstring for a named symbol. Pass symbol='ClassName' or symbol='method_name'. No line number needed. callers: every call site of a named symbol, grouped by file with source context. definitions: where a symbol used in this file was defined (follows imports across files). problems: type errors and warnings in a file. rename: preview all sites that would change if a symbol were renamed. No writes applied.")
    path: str = Field(default='.', description="File (or directory for 'map') relative to cwd.")
    symbol: str | None = Field(default=None, description="Symbol name for inspect / callers / definitions / rename. Use the unqualified name: 'search_file' not 'SymbolFinder.search_file'. Not needed for map or problems.")
    new_name: str | None = Field(default=None, description='Replacement name, required for rename.')
    lang: str | None = Field(default=None, description="Override language detection, e.g. 'py', 'ts'.")
    context_lines: int = Field(default=4, description='Source lines of context around each caller hit. Default 4.')
...
class LspTool(Tool):
    ...
    async def execute(self, params: LspParams, signal=None) -> dict:
        ...
        if lang is None and (not abs_path == cwd):
            if pathlib.Path(abs_path).is_dir():
                ...
                return tout(f'Unsupported file type: {pathlib.Path(params.path).suffix or '(no extension)'}', True)
        ...
        try:
            ...
            if client is None:
                ...
                return tout(f"No LSP server found for '{lang}'. Install one of: {(', '.join(names) if names else '(none configured)')}", True)
            if params.action == 'problems':
                return await self._problems(client, abs_path, rel)
            if not params.symbol:
                return tout(f"'{params.action}' requires 'symbol'. Run map first to see available symbols.", True)
    ...
    async def _map(self, abs_path: str, rel: str, cwd: str, lang: str | None, params: LspParams) -> dict:
        ...
        if client is None:
            ...
            return tout(f"No LSP server found for '{file_lang}'. Install: {', '.join(names)}", True)
    ...
    async def _inspect(self, client: LspClient, abs_path: str, rel: str, cwd: str, params: LspParams) -> dict:
        syms = await _get_hierarchy(client, abs_path)
        sym = _find_symbol_by_name(syms, params.symbol)
        if sym is None:
            return tout(f"Symbol '{params.symbol}' not found in {rel}. Run map to see available symbols.", True)
        s, e = _sym_range(sym)
    ...
    async def _callers(self, client: LspClient, abs_path: str, rel: str, cwd: str, params: LspParams) -> dict:
        syms = await _get_hierarchy(client, abs_path)
        sym = _find_symbol_by_name(syms, params.symbol)
        if sym is None:
            return tout(f"Symbol '{params.symbol}' not found in {rel}. Run map to see available symbols.", True)
        ...
        for uri in sorted(by_file):
            ...
            for r in sorted(frefs, key=lambda x: x.get('range', {}).get('start', {}).get('line', 0)):
            ...
            for win_s, win_e, win_refs in windows:
                hit = {r.get('range', {}).get('start', {}).get('line', 0) for r in win_refs}
                parts.append('')
    ...
    async def _definitions(self, client: LspClient, abs_path: str, rel: str, cwd: str, params: LspParams) -> dict:
        ...
        for loc in defs:
            ...
            try:
                ...
                if enclosing:
                    ...
                    snippet = '\n'.join((f'{('>' if i == def_line_0 else ' ')} {i + 1:5d}  {src_lines[i]}' for i in range(lo, hi)))
    ...
    async def _rename(self, client: LspClient, abs_path: str, rel: str, cwd: str, params: LspParams) -> dict:
        ...
        if not edit:
            return tout(f"Server returned no rename edit for '{name}'. Symbol may not be renameable.", True)
        formatted, n_sites = _fmt_rename_edit(edit, cwd)
        return tout(f"Rename '{name}' → '{params.new_name}' — {n_sites} site(s):\n{formatted}")
