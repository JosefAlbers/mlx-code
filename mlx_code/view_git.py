...
def _parse_agent_branch(name: str) -> str | None:
    if name.startswith(('agent/', 'resume/')):
        seg = name.split('/', 1)[1]
        dash = seg.find('-')
        return seg if dash == -1 else seg[:dash]
    if '--' in name:
        seg = name.split('--', 1)[1]
        dash = seg.find('-')
        return seg if dash == -1 else seg[:dash]
...
def build_tree(repo_dir: str, repo: _git.Repo | None=None) -> list[CommitNode]:
    ...
    agent_branches = [b for b in repo.branches if _parse_agent_branch(b.name) is not None]
    if not agent_branches:
        return []
    base_map: dict[str, list] = {}
    for b in agent_branches:
        base_hex = _parse_agent_branch(b.name)
        try:
    ...
    for hexsha in base_map:
        try:
            ...
            base_dates[hexsha] = 0
    sorted_bases = sorted(base_map.keys(), key=lambda h: base_dates.get(h, 0), reverse=True)
    ...
    for base_hex in sorted_bases:
        try:
            ...
            continue
        base_node = CommitNode(hexsha=bc.hexsha, short=_short(bc.hexsha), summary=bc.summary[:60], committed_dt=_fmt_dt(bc), author=bc.author.name or '?', kind='base', depth=0)
        branches = sorted(base_map[base_hex], key=lambda b: b.commit.committed_date, reverse=True)
        for br in branches:
            try:
            ...
            tip = br.commit
            label = CommitNode(hexsha=tip.hexsha, short=_short(tip.hexsha), summary=br.name, committed_dt=_fmt_dt(tip), author=tip.author.name or '?', kind='branch_label', branch_name=br.name, parent_hexsha=base_hex, depth=1)
            for i, c in enumerate(branch_commits):
                parent_hex = branch_commits[i - 1].hexsha if i > 0 else base_hex
                label.children.append(CommitNode(hexsha=c.hexsha, short=_short(c.hexsha), summary=c.summary[:60], committed_dt=_fmt_dt(c), author=c.author.name or '?', kind='commit', branch_name=br.name, parent_hexsha=parent_hex, depth=2))
...
def get_diff(repo: _git.Repo, node: CommitNode, mark_node: CommitNode | None=None) -> list[str]:
    try:
...
def get_stat(repo: _git.Repo, node: CommitNode, mark_node: CommitNode | None=None) -> str:
    try:
        if mark_node and mark_node.hexsha != node.hexsha:
            ...
            if parent_hex:
                ...
                commit = repo.commit(node.hexsha)
                if commit.parents:
                    return repo.git.diff(commit.parents[0].hexsha, node.hexsha, '--stat')
                else:
...
def restore_snapshot(repo: _git.Repo, node: CommitNode, base_dir: str) -> tuple[bool, str]:
...
class State:
    ...
    _diff_cache: tuple[str, str | None, list[str], str, list[str]] | None = field(default=None, repr=False)
    ...
    def get_diff_cached(self) -> list[str]:
        ...
        if self._diff_cache is not None and self._diff_cache[0] == node.hexsha and (self._diff_cache[1] == mark_sha):
    ...
    for i, node in enumerate(visible[start:start + rows]):
        ...
        if node.kind == 'base':
            ...
            for vi in range(abs_i - 1, -1, -1):
                if visible[vi].kind == 'branch_label' and visible[vi].branch_name == node.branch_name:
...
def draw_diff(win, state: State, h: int, w: int):
    ...
    lines.append((hdr.ljust(w - 1), curses.color_pair(C_HEADER) | curses.A_BOLD | curses.A_REVERSE))
    ...
    if max_scroll > 0:
        ...
        indicator = f' {pct}% '
        try:
            win.addstr(h - 1, max(0, w - len(indicator) - 1), indicator, curses.color_pair(C_DIM))
        except curses.error:
            pass
    win.noutrefresh()
...
def diff_pager(stdscr, state: State):
    ...
    if msg_lines:
        lines.append(('', C_NORMAL))
        lines.append((' ── commit message ' + '─' * 40, curses.color_pair(C_DIM) | curses.A_BOLD))
        for ml in msg_lines:
    ...
    while True:
        ...
        try:
            stdscr.addstr(0, 0, header[:w - 1].ljust(w - 1), curses.color_pair(C_SELECTED) | curses.A_BOLD)
        except curses.error:
            pass
        ...
        if total > visible:
            ...
            try:
                stdscr.addstr(h - 1, 0, hint[:w - 1].ljust(w - 1), curses.color_pair(C_SELECTED))
            except curses.error:
                pass
        stdscr.noutrefresh()
        curses.doupdate()
...
def draw_statusbar(stdscr, state: State, h: int, w: int):
    ...
    if state.status_msg:
        ...
        attr = curses.color_pair(C_STATUS_OK) if state.status_ok else curses.color_pair(C_STATUS_ER)
    else:
        parts = []
        if node:
        ...
        parts.append('j/k:move  l/h:expand  E:expand-all  v:mark  Esc:clr/help  r:restore  o:fullscreen  q:quit  ?:help')
        msg = '  '.join(parts)
...
def run(stdscr, repo_dir: str, refresh_secs: float=0.0):
    ...
    _refresh_ticks = max(1, int(refresh_secs * 1000 / _tick_interval_ms)) if refresh_secs > 0 else 0
    _poll_counter = 0
    ...
    while True:
            ...
            if _refresh_ticks:
                ...
                if _poll_counter >= _refresh_ticks:
                    ...
                    if mtime != _last_mtime:
                        _last_mtime = mtime
                        cur_sha = state.current_node().hexsha if state.current_node() else None
                        try:
                            fresh_repo = _git.Repo(repo_dir, search_parent_directories=True)
                        ...
                        if new_roots is not None and fresh_repo is not None:
                            ...
                            expanded_shas, expanded_branches = _collect_expanded(state.roots)
        ...
        if key in (ord('j'), curses.KEY_DOWN):
            ...
            if node:
                ...
                state.clamp_cursor()
                state.invalidate_diff()
                state.status_msg = f'expanded {n} node{('s' if n != 1 else '')} under cursor'
            ...
            break
    return (state.quit_snapshot_path, state.mark_node.hexsha if state.mark_node else None)

def main():
    import argparse
    parser = argparse.ArgumentParser(description='view_git – worktree snapshot navigator')
    parser.add_argument('repo_dir', nargs='?', default=os.getcwd())
    parser.add_argument('--refresh', '-R', type=float, default=0.0, metavar='SECONDS', help='poll .git for changes every N seconds and rebuild tree (0 = disabled)')
