        ...
        for b in content:
            ...
            if btype == "thinking":
                thinking_parts.append(
                    b.get("thinking") or b.get("reasoning_content") or b.get("text", "")
                )
            elif btype == "text":
    ...
    if isinstance(sys, str) and sys.strip():
        ...
        text = "\n\n".join(
            (
                ...
                for b in sys
                if b.get("type") == "text"
                and (not b["text"].startswith("x-anthropic-billing-header:"))
            )
        )
        if text:
            messages.append(Message(role="system", content=text))
    ...
    for msg in messages:
        if msg.role == "tool":
            ...
            m = {
                ...
                "content": f"<input>{json.dumps({'name': tc.name, 'arguments': tc.arguments})}</input>\n{content}"
                if tc and (not render_tc)
                else content,
            }
            out.append(m)
        else:
...
def translate(
    ...
    **kwargs,
    ...
    if system_override is not None:
        messages = [
            replace(m, content=system_override) if m.role == "system" else m
            for m in messages
        ]
...
def encode(body, api, tokenizer, system_override, tool_names, skips, strict=False):
    ...
    if not msgs or not msgs[-1].get("content", "").strip():
        return ("", None)
    apply_chat_template = lambda x: tokenizer.apply_chat_template(
        x, tools=tools or None, tokenize=False, add_generation_prompt=True
    )
    full_s = apply_chat_template(msgs)
    add_special_tokens = tokenizer.bos_token is None or not full_s.startswith(
        tokenizer.bos_token
    )
    ...
    for last_user_idx in (i for i, m in enumerate(msgs) if m.get("role") == "user"):
        p_msgs = msgs[:last_user_idx] + [
            dict(
                role="user",
                content="h" if msgs[last_user_idx]["content"][0] != "h" else "i",
            )
        ]
    ...
    logger.debug(
        ...
        + "\n".join(
            [
                f"{tokenizer.decode(full[i:j])}\n---{j}"
                for i, j in zip([0] + sorted(ckpts), sorted(ckpts) + [len(full)])
            ]
        )
    )
...
class PromptCache:
    ...
    def __call__(self, prompt, ckpts):
        ...
        logger.debug(
            f"ckpts={ckpts!r} len(prompt)={len(prompt)!r} len(self.hx)={len(self.hx)!r} cl={cl!r}"
        )
    ...
    def load(self, prompt):
        ...
        logger.debug(path)
        self.cache, metadata = mlx_lm.models.cache.load_prompt_cache(
...
def generate(model, tokenizer, prompt, ckpts, pc, max_tokens=256, **kwargs):
    ...
    token_gen = generate_step(
        ...
        save_fn=save_fn,
        _te=tokenizer.convert_tokens_to_ids("</think>"),
        **kwargs,
    )
    ...
    for token, _ in token_gen:
        ...
        if len(gens) == 1:
            ...
            _log_str += "\n".join(
                [
                    f"{tokenizer.decode(prompt[i:j])}"
                    if j == len(prompt)
                    else f"{tokenizer.decode(prompt[i:j])}\n--- {j}"
                    for i, j in zip([0] + sorted(ckpts), sorted(ckpts) + [len(prompt)])
                ]
            )
        if seg:
            yield seg
    ...
    *,
    ...
    if input_embeddings is not None:
        if not does_model_support_input_embeddings(model):
            ...
            raise ValueError(
                f"len(input_embeddings)={len(input_embeddings)!r} len(prompt)={len(prompt)!r}"
            )
    elif len(prompt) == 0:
        raise ValueError(
    ...
    with mx.stream(generation_stream):
        ...
        while total_prompt_tokens - prompt_processed_tokens > 1:
            ...
            _model_call(
                ...
                input_embeddings=input_embeddings[:n_to_process][None]
                if input_embeddings is not None
                else None,
            )
            quantize_cache_fn(prompt_cache)
...
def _parse_tools_xml(part):
    ...
    for match in tool_pattern.finditer(part):
        ...
        if fn_match:
            ...
            params = re.findall(
                "<parameter=([^\\s>]+)>\\s*(.*?)\\s*</parameter>", content, re.DOTALL
            )
            blocks.append(
                {
...
class ClaudeAdapter(BaseAdapter):
    ...
    def _start_block(self, state, extra=None):
        ...
        return self._event(
            "content_block_start",
            {"type": "content_block_start", "index": self.index, "content_block": cb},
        )
    ...
    def _delta(self, state, text):
        return self._event(
            "content_block_delta",
            {
                ...
                "index": self.index,
                "delta": {"type": f"{state}_delta", state: text},
            },
        )
    ...
    def tool(self, tool):
        ...
        out += self._start_block(
            "tool_use", {"id": tool["id"], "name": tool["name"], "input": {}}
        )
        out += self._event(
            "content_block_delta",
            {
...
class DefaultAdapter(BaseAdapter):
    ...
    def tool(self, tool):
        ...
        return self.chunk(
            ...
            {
                "tool_calls": [
                    {"index": i, "function": {"arguments": json.dumps(tool["input"])}}
                ]
            }
        )
...
def make_handler(
    model_name, cache_dir, system, names, skips, gwt=None, parse_think=True
):
                                ...
                                "candidates": [
                                    {
                                        "content": {
                                            ...
                                            "parts": [
                                                {
                                                    "text": '{"complexity_reasoning":"local","complexity_score":50}'
                                                }
                                            ],
                                        },
                                        "finishReason": "STOP",
...
def main():
    ...
    parser.add_argument(
        "-l",
        "--leash",
        choices=["claude", "codex", "gemini", "noapi", "none"],
        default="noapi",
        help="AI harness to launch against the server; 'noapi' starts the built-in REPL, 'none' runs the server only",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="System prompt override passed to the model",
    )
    parser.add_argument(
        "--cache",
        type=str,
        default=".cache",
        help="Directory for the KV-cache (default: .cache)",
    )
    parser.add_argument(
        "--work",
        default=os.getcwd(),
        help="Working directory used as the git repo root (default: cwd)",
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Port to listen on; auto-increments if already in use (default: 8000)",
    )
    parser.add_argument(
        "--skips",
        nargs="+",
        default=[
            "(?m)^\\[SUGGESTION MODE[\\s\\S]*",
    ...
    with tempfile.TemporaryDirectory(dir="/tmp") as _home:
        ...
        gwt = (
            None
            if args.leash in ("none", "noapi")
            else create_worktree(args.work, worktree_dir=str(home / "workspace"))
        )
        ...
        if args.leash == "none":
            ...
            if args.leash == "noapi":
                ...
                sys.exit(
                    subprocess.run(
                        [args.leash] + leash_args, env=env, cwd=cwd
                    ).returncode
                )
