...
class KB:
    ...
    def __init__(self, src_dir: str | Path | None=None, db_path: str | Path | None=None):
        ...
        if self.source_dir:
            for path in self.source_dir.rglob('*'):
                ...
                if rel in self.db:
                    continue
                self.db[rel] = {'id': rel, 'parent': None, 'children': [], 'content': path.read_text(errors='ignore')}
        self.save()

    def __call__(self, content: str, parent: str | None=None, id: str | None=None, id_prefix: str | None=None) -> str:
    ...
    def get_branch(self, id: str, overrides: dict[str, str] | None=None, indent: bool=True) -> str:
        ...
        def branch_format(node: dict[str, Any]) -> str:
            ...
            inner_body = content + (children if not children else '\n' + children.strip())
    ...
    def get_discussion(self, id: str) -> str:
        ...
        for node in chain:
            ...
            xml_elements.append(f'<comment id="{node['id']}" parent="{parent_id}">\n{node['content'].strip()}\n</comment>')
        return '\n'.join(xml_elements)
    ...
    def get_revision(self, id: str, raw: bool=True) -> str:
        ...
        for i in range(len(chain) - 1):
            ...
            if i == len(chain) - 2:
                diff_lines += list(difflib.unified_diff(a_lines, b_lines, fromfile=rev_map[a['id']] if not raw else a['id'], tofile=rev_map[b['id']] if not raw else b['id'], lineterm='', n=0))
            else:
                raw_diff = list(difflib.unified_diff(a_lines, b_lines, lineterm=''))
                added = sum((1 for l in raw_diff if l.startswith('+') and (not l.startswith('+++'))))
                removed = sum((1 for l in raw_diff if l.startswith('-') and (not l.startswith('---'))))
        ...
        diff_block = f'<diff {attr(last['id'])}>\n' + '\n'.join(diff_lines) + '\n</diff>'
        return f'{doc}\n{diff_block}'
    ...
    def up(self, id: str) -> dict[str, Any] | None:
        if id is None:
            return None
        node = self.db[id]
        return {'id': node['id'], 'content': node['content'], 'parent': self.up(node['parent'])}

    def down(self, id: str) -> dict[str, Any]:
        node = self.db[id]
        return {'id': node['id'], 'content': node['content'], 'parent': node['parent'], 'children': [self.down(child_id) for child_id in node['children']]}
...
class DocThread:
    ...
    def submit(self, content: str, parent: str | None=None, inplace: bool=True) -> str:
        parent_doc_id = None
        root_id = None
        if parent is not None:
    ...
    def read(self, comment_id: str) -> str:
        ...
        return self.kb.get_branch(comment_id, overrides={root_id: self.kb.get_revision(doc_id, raw=False)})
