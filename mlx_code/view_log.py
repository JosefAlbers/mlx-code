...
def pager(stdscr, entry: dict):
    ...
    while True:
        ...
        try:
            stdscr.addstr(
                0, 0, header[: w - 1].ljust(w - 1), curses.color_pair(8) | curses.A_BOLD
            )
        except curses.error:
            pass
        ...
        if total > visible:
            ...
            try:
                stdscr.addstr(
                    h - 1, 0, hint[: w - 1].ljust(w - 1), curses.color_pair(9)
                )
            except curses.error:
                pass
        stdscr.noutrefresh()
...
def render_list(
    ...
    for idx in range(visible - 1):
        ...
        if marked_indices is not None and entry_to_master_index is not None:
            ...
            in_v_range = v_mode and min(v_anchor, cursor) <= entry_idx <= max(
                v_anchor, cursor
            )
            marker = ">" if is_marked or in_v_range else " "
            marker_color = (
                curses.color_pair(3) | curses.A_BOLD
                if in_v_range and (not is_marked)
        ...
        is_related = (
            ...
            and (
                (related_keys[entry_idx] if related_keys else _related_key(entry))
                == cursor_key
            )
        )
        if is_sel:
            try:
                win.addstr(
                    row, 2, f"{line:<{w - 4}}", curses.color_pair(6) | curses.A_BOLD
                )
            except curses.error:
                pass
        elif is_related:
...
def tui(stdscr, entries, log_file, initial_filter="", initial_visible=None):
    ...
    while True:
        ...
        if key in (ord("q"), 27):
            ...
            h, w = stdscr.getmaxyx()
            list_win, detail_win, list_w, detail_w, pane_h, col_widths = _make_windows(
                h, w
            )
            stdscr.clearok(True)
...
def main():
    ...
    parser.add_argument(
        "-f",
        "--filter",
        default=f"lvl:10;file:main,repl,gits,apis,tools",
